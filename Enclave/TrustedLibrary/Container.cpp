...
std::unordered_set<uint32_t> containers::find_sim(uint64_t query[])
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		for (auto temp : map2liner)
		{
			...
			for (int t = 0; t < 4; t++)
			{
				tmp_size += ((uint32_t)(sub_identifiers[i][temp->begin + t]) << (8 * t));
			}
			// printf("tmp_size: %u\n",tmp_size);
			if (tmp_size <= COMPRESS_MIN)
			{
				...
				for_uncompress(sub_identifiers[i].data() + temp->begin + 4, out_tmp, tmp_size); // decompress
		...
		{ // auto temp:miss_sub
			...
			for (int t = 0; t < 4; t++)
			{
				tmp_size += ((uint32_t)(sub_identifiers[i][its->begin + t]) << (8 * t));
			}
			// printf("tmp_size: %u\n",tmp_size);
			...
			if (tmp_size <= COMPRESS_MIN)
			{
				...
				for_uncompress(sub_identifiers[i].data() + its->begin + 4, out_tmp, tmp_size); // decompress
