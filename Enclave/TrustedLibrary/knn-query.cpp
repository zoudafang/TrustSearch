...
std::vector<std::pair<uint32_t, uint32_t>> containers::find_knn(uint64_t query[], int KNN_NUM)
{
    ...
    // for (uint32_t ham_dist = 0; ham_dist < search_max; ham_dist++)
    // {
    //     for (int i = 0; i < SUBINDEX_NUM; i++)
    //     {
    //         lookup_all_size = 0;
    ...
    //             for (int x = 0; x < s; x++)
    //                 power[x] = x;    // power[i] stores the location of the i'th 1
    ...
    //                     if (filters.contains(bloom_hash, bloom_hash_times * uint_size)) // filters.contains(bloom_hash, bloom_hash_times * uint_size)
    //                     {
    //                         existed_subkeys.push_back(key_find{tmpsub1, (uint16_t)ham_dist, (uint16_t)find_max_d});
    //                     }
    ...
    //         }
    //         if (lookup_all_size >= ceil((double)sub_linear_comp[i].size() / 3)) // lookup_all_size >= (sub_linear_comp[i].size() >> 1) lookup_all_size >= ceil((double)sub_linear_comp[i].size() / 3)
    //         {
    ...
    //                             get_cand_knn({tmp.sub_key, tmp.skiplen, tmp.length}, i, query, &knn_res, KNN_NUM);
    //                         }
    //                     }
    ...
    //                     get_cand_knn({tmpsub1, its->skiplen, its->length}, i, query, &knn_res, KNN_NUM);
    //                 }
    //             }
    //         }
    //         else
    ...
    //         // // two many data in enclave
    //         // if (cand_filters.size() > 500000)
    //         // {
    //         //     cand_filters.clear();
    //         // }
    //     }
...
void containers::get_cand_knn(sub_info_comp comp, uint32_t i, uint64_t *query, void *res, uint32_t KNN_NUM)
{
