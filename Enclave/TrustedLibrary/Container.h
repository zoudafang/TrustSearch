...
public:
	...
	int successful_num=0;
	set<uint32_t> candidate;
	forward_list<full_information> full_index;
	forward_list<sub_information>sub_index1;
	forward_list<sub_information>sub_index2;
	forward_list<sub_information>sub_index3;
	forward_list<sub_information>sub_index4;
	vector<uint32_t>C_0_TO_subhammdis; //用于与特征段做异或运算的所有数字的容器
	set<pair<uint64_t,uint64_t>>test_pool;
	containers();
	void random_128(uint64_t *temp_key);
