...
public:
   ...
   inline void insert(const unsigned char *key_begin, const std::size_t &length)
   {
      ...
      for (std::size_t i = 0; i < salt_.size(); ++i)
      {
         ...
         // printf("len%d hash %d\n",length,salt_.size());
         // char* tmp=key_begin;
         bit_index = *(reinterpret_cast<const unsigned int *>(key_begin + i * sizeof(uint32_t))); //(tmp+i*4);
         bit_index %= table_size_;
   ...
   inline virtual bool contains(const unsigned char *key_begin, const std::size_t length) const
   {
      ...
      for (std::size_t i = 0; i < salt_.size(); ++i)
      {
         ...
         bit_index = *(reinterpret_cast<const unsigned int *>(key_begin + i * 4));
         bit_index %= table_size_;
         bit = bit_index % bits_per_char;
