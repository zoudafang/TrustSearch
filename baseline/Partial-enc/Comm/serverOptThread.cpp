...
void ServerOptThread::Run(SSL *clientSSL)
{
    ...
    uint32_t dataSize = 0;
    recvArrSize = QUERY_SIZE * sizeof(uint32_t);
    ...
    while (true)
    {
        ...
        case QUERY_BATCH:
        {
            ...
            Con::cont.partQuery(queryPart, result, PLAIN_LEN, hammdist, client_id);
            uint8_t *dataPtr = reinterpret_cast<uint8_t *>(result.data());
            printf("server send size %lld\n", result.size() * sizeof(triRes));

            dataSecureChannel_->SendData(clientSSL, dataPtr, result.size() * sizeof(triRes));
