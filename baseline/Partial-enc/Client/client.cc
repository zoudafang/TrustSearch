    ...
    for (int h = 1; h < 2; h++)
    {
        // for (int times = 0; times < 1000; times++)
        {
            ...
            for (int t = 0; t < 1000; t++)
            {
                ...
                int itemNum = *(uint32_t *)(data2), distP, dist;
                uint8_t *encData;
                uint8_t *constFeatureKey = feature_key, *dataTriRes = data2 + sizeof(triRes);
                // printf("le %d\n", itemNum);
                ...
                // printf("dec time：%f秒\n", durationE2.count());
                printf("hamming %d success_num:%d,client: %f秒 server: %f ,total-time %f \n",
                ...
                // printf("%f \n", duration2.count());
                time_list.push_back(durationE2.count() + durationE.count());
            }
            printf("TTTThamming %d success_num:%d,server: %f秒 client: %f ,total-time %f \n",
