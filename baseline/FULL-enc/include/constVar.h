...
static const char SERVER_IP[] = "127.0.0.1";                // client和server通信的IP和Port
static const int SERVER_PORT = 9030;
static const int GC_SERVER_PORT = 9099;
static const uint32_t THREAD_STACK_SIZE = 8 * 1024 * 1024;

static const uint32_t VECTOR_BIT = 128;
static const uint32_t PLAIN_BIT = 128; // 96  128 bug?
...
static const uint32_t SUBINDEX_NUM = 15; // 15;
...
static uint32_t CLIENT_NUM = 1;

// -- -- -- -- -- -- -- --EncryptGram------------

static const uint32_t KEY_BIT = 512;
static const uint32_t MASK_R_BIT = 256;
static const uint32_t KEY_LEN = KEY_BIT / 8 * 4; // bit/8=len;len*2*2=enc_msg_len
static const uint32_t RAND_KEY_BIT = 256;
static const uint32_t RAND_KEY_LEN = RAND_KEY_BIT / 8;
static const uint32_t HASH_KEY_BIT = 256;
static const uint32_t HASH_KEY_LEN = HASH_KEY_BIT / 8;

static const uint32_t RAND_PARAM_LEN = 32 / 8;

static const uint32_t QUERY_BUF_LEN = RAND_PARAM_LEN + RAND_PARAM_LEN + KEY_LEN;
static const uint32_t QUERY_BUF_IDX = QUERY_BUF_LEN + sizeof(uint32_t);
...
typedef struct
{
    ...
    // Prf *G2Prf;
    // uint8_t *encFullkey;
    uint8_t *dataBuffer; // f1prf+g2prf+encfullkey
} QueryBuffer;
typedef struct
{
    QueryBuffer qbf;
...
static const uint32_t NearJ_SIZE = sizeof(int) + sizeof(int) + KEY_LEN;
struct NearJ
{
    uint32_t img_id;
    int next_pos;
    uint8_t encFullkey[KEY_LEN] = {0};

    std::vector<uint8_t> tobytes()
    {
        std::vector<uint8_t> res(sizeof(img_id) + sizeof(next_pos) + KEY_LEN);
        int l1 = sizeof(img_id), l2 = sizeof(next_pos), l3 = KEY_LEN;
        (uint32_t &)res[0] = img_id;
        (uint32_t &)res[l1] = next_pos;
        memcpy(res.data() + l1 + l2, encFullkey, l3);
        return res;
    }
    static inline uint32_t bytesLen() { return sizeof(img_id) + sizeof(next_pos) + KEY_LEN; }
};
struct RandKey
{
    uint8_t rk[RAND_KEY_BIT / 8];
};
...
static const uint32_t QUERY_SIZE = 10000000;
static const uint32_t QUERY_BATCH_NUM = QUERY_SIZE / QUERY_BUF_IDX - 100;
