	...
	for (int i = 0; i < SUBINDEX_NUM; i++)
	{
		...
		if (1) // lookup_all_size >= (sub_linear_comp[i].size() >> 1) lookup_all_size >= ceil((double)sub_linear_comp[i].size() / 3)
		{
			for (auto val = tmp_clrs.begin(); val < tmp_clrs.end();)
			{
				...
				end_idx = val->end;
				if ((val->dist + max_dist - 1) <= sub_hammdist[i] || tmp_node.is_combined) // end_idx - begin_idx < 500
				{
					linear_scan(i, begin_idx, end_idx, sub[i], sub_hammdist[i], visited_keys, candidate, reached_subkey);
		...
		else
		{
			...
			for (int t = 0; t < max_node; t++)
			{
				...
				else
				{
					...
					for (int j = 0; j < existed_subkeys.size(); j++) // exist-keys 有很多，但是实际上只需要在与它最近的cluster中find
					{
						...
						if (its != sub_linear_comp[i].end() && (its->sub_key == tmpsub1 || its->length & MASK_INF)) //&& its->sub_key == tmpsub1
						{
							if (its == sub_linear_comp[i].begin() + end_idx && its->length & MASK_INF)
							{
								big_uneq++;
							}
							if (its->sub_key == tmpsub1)
			...
			if (min_dist0 + sub_hammdist[i] > max_dist)
			{
				...
				if (begin_idx < end_idx) // cautious for stash==0
				{
					...
					{
						auto &tmpsub1 = val.subkey;
						if (val.dist < dt1 || val.max_dist < max_dist) //|| visited_subkeys.find(tmpsub1) != visited_subkeys.end()
							continue;
						if (reached_subkey.find(tmpsub1) != reached_subkey.end())
							continue;
	...
	search_end:
		...
		for (int y = 0; y < visited_keys.size(); y += 1)
		{
			...
			if (val != reached_subkey.end() && val->second == -1)
				continue;
			gen_candidate(candidate, visited_keys[y], tmp_visit, tmpv, i, sub[i], 100);
		}
		visited_keys.clear();
		ocall_get_timeNow(time);
		end_time = *time;
...
void containers::gen_candidate(std::unordered_set<uint32_t> &cand, sub_info_comp comp, vector<sub_info_comp> &visited_keys, vector<sub_info_comp> &tmp_keys,
							   uint32_t i, uint32_t subkey, uint32_t dt)
{
	...
	else
	{
		...
		{
			cand.emplace_hint(cand.begin(), out_tmp[j]); // cautious for cost too much time
		}
	}
...
void containers::linear_scan(uint32_t i, uint32_t begin, uint32_t end, uint32_t subkey, uint32_t hammdist,
							 std::vector<sub_info_comp> &visited_keys, unordered_set<uint32_t> &candidate, std::unordered_map<uint32_t, int> &reached_subkey)
{
	sub_info_comp tmp_info;
	for (uint32_t c = begin; c < end; c++)
	{
		...
		if (tmp_info.length & MASK_INF)
		{
			...
			uint64_t key = ((uint64_t)i << 32) | tmp_info.skiplen;
			auto val = data_cache.find(key);
			// if (val != data_cache.end())combs_hit += val->second->key;
			if (val != data_cache.end())
			{
			...
			uint32_t tempKey = tmp_info.sub_key;
			uint32_t tmp_size = 0;
			int tmp_begin = 0;
			bool is_combined_keys = false;
...
void init_after_send()
{
	...
	cont.lru_cache.capacity = 500000; // 5000 ((uint32_t)floor((double)nums / 100) < 20000 ? 20000 : (uint32_t)floor((double)nums / 100)); //	nums + 100000; //
	...
	for (int i = 0; i < SUBINDEX_NUM; i++)
	{
		printf("sub-linear %d sub-comp %d size %d\n", i, cont.sub_index_liner[i].size(), cont.sub_linear_comp[i].size());
	}
	printf("\n");
	printf("The full index entry is: %d \n", cont.full_index.size());
	printf("The number of queries is: %d \n", cont.test_pool.size());
...
void containers::make_clusters()
{
	...
	for (int i = 0; i < SUBINDEX_NUM; i++)
	{
		...
		for (int c_idx = 0; c_idx < (clr[i].size()); c_idx++)
		{
			...
			bool is_comb_clr = ((end_idx - j) > combine_clr_min ? true : false); // ((end_idx - j) > combine_clr_min ? true : false); //(c_idx == clr[i].size() - 1 || (end_idx - j) <= combine_clr_min ? false : true); //
			if (!is_comb_clr)
			...
			for (; j < end_idx; j++)
			{
				...
				else
				{
					...
					// combine subkey后产生的block大小不应该大于aggre_size
					if ((temp_vec.size() + 1 + same_num) < PAGE_SIZE)
					{
						num2++;
...
// 	}
// }
void ecall_change_para(uint32_t dataSet, uint32_t hamm, uint32_t clr_size, uint32_t clr_dist, uint32_t comb_num, uint32_t aggre_size)
{
	cont.aggre_size = aggre_size;
	cont.hammdist = hamm;
...
vector<uint32_t> containers::get_rand_keys(int i, int k, vector<uint32_t> &old_keys)
{
	...
	// 		{
	// 			dist_sum += popcount(val ^ sub_index_liner[i][index].sub_key);
	...
	while (1)
	{
		...
		ridx = ridx % sub_index_liner[i].size();
		...
		if (keys.size() < k)
		{
			keys.push_back(sub_index_liner[i][ridx].sub_key);
		}
		else
		{
			break;
...
vector<cluster_node> containers::kmodes(int i)
{
	...
	while (times < 50) // 50
	{
		...
		for (int j = 0; j < sub_index_liner[i].size(); j++)
		{
			...
			}
			{
				...
				if (tmp_dist <= max_dist)
				{
					...
					for (int t = 0; t < 32; t++)
					{
						if ((sub_index_liner[i][j].sub_key >> t) & 1)
		...
		// else
		{
			...
			// 	{
			// 		if (popcount(keys[t1] ^ tmp_clrs[t].second) < max_dist)
			// 			goto loop;
			// 	}
			...
			if (stash_nums >= ceil((double)sub_index_liner[i].size() / 2))
			{
				auto tmp = get_rand_keys(i, k, keys);
				unordered_set<uint32_t> tmp_set(keys.begin(), keys.end());
				for (auto &val : tmp)
				{
					// 	for (auto &val1 : keys)
					// 	{
					// 		if (popcount(val ^ val1) < (7))
					// 			goto loop2;
					// 		if (keys.size() >= k + 10)
					// 			break;
					// 	}
					...
					// 	if (keys.size() >= k + 10)
					// 		break;
					if (tmp_set.find(val) == tmp_set.end() && keys.size() < k + 10) // 20//5
						keys.push_back(val);
				}
			}
			// printf("add keys %d\n", keys.size());
...
uint8_t *containers::lru_ids_add(uint64_t key, uint32_t sub_i, sub_info_comp comp)
{
	...
	if (remove_node == nullptr)
	{
		...
		node->ids.resize(ceil((double)max_id_page / 2)); // / 2
	}
	else
		node = remove_node;
