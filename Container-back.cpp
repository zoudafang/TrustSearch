...
std::vector<uint32_t> containers::find_sim(uint64_t query[], uint32_t tmp_test_target)
{
	...
	for (int i = 0; i < SUBINDEX_NUM; i++)
	{
		...
		lookup_all_size = 0;
		lookup_radius = sub_hammdist[i] + max_dist;
		get_times(1, 0);
		tmp_dist = 0;
		dt = 0;
		tmp_visit.clear();
		...
		for (int t = 0; t < clr[i].size() - 1; t++)
		{
			...
			if (tmp_dist > sub_hammdist[i] + max_dist - 1) //- 2
			{
				// if (tmp_dist == sub_hammdist[i] + max_dist - 1)
				// {
				// 	mid_clrs.push_back(c_info);
				// }
				// else
				if (tmp_dist == sub_hammdist[i] + max_dist)
				{
		...
		{
			// if (tmp_clrs.size() == 1 && mid_clrs.size()) // tmp-clrs只有stash，可以从bigger里面查找
			// {
			...
			// }
			// else
			if (tmp_clrs.size() == 1 && bigger_clrs.size()) // tmp-clrs只有stash，可以从bigger里面查找
			{
				tmp_node = bigger_clrs[0].node;
				begin_idx = tmp_node.begin_idx;
				end_idx = bigger_clrs[0].end;
				auto its = std::lower_bound(sub_linear_comp[i].begin() + begin_idx, sub_linear_comp[i].begin() + end_idx, tmpkey, compareFirst_comp);
				if (its < (sub_linear_comp[i].begin() + end_idx) && (its->sub_key == tmpkey || its->length & MASK_INF)) //&& its->sub_key == tmpsub1
				{
					key_find kf{0, 0, 0};
		...
		for (auto val = tmp_clrs.begin(); val < tmp_clrs.end();)
		{
			...
			if ((val->dist + max_dist) <= sub_hammdist[i] || val->node.group_size < combine_clr_min[i]) // end_idx - begin_idx < 500 (val->dist + max_dist - 1) <= sub_hammdist[i] || val->node.group_size < combine_clr_min
			{
				linear_scan(i, begin_idx, end_idx, sub[i], sub_hammdist[i], candidate, reached_subkey);
		...
		if (1) // lookup_all_size >= (sub_linear_comp[i].size() >> 1) lookup_all_size >= ceil((double)sub_linear_comp[i].size() / 3)
		{
			...
			for (int x = 0; x < existed_subkeys.size(); x++)
			{
				...
				{
					// search in stash
					existed_subkeys[x].clr_idx = tmp_clrs.size(); // too minor，不要随便乱改字段意义啊喂
				}
			}
			c_info.node = clr[i][clr[i].size() - 1];
			c_info.end = sub_linear_comp[i].size();
			...
			/*	uint32_t mid_idx, mid_dist;
				for (auto &val : existed_subkeys)
				{
					...
					if (val.dist == sub_hammdist[i] - 1 && val.max_dist >= max_dist) //!=0
					{
					...
					else if (val.dist == sub_hammdist[i] && val.max_dist >= max_dist) //!=0
					{
			...
			for (auto &val : existed_subkeys)
			{
				if (val.dist == sub_hammdist[i] && val.max_dist >= max_dist) //!=0
				{
			...
			if (min_dist0 + sub_hammdist[i] > max_dist)
			{
				...
				if (begin_idx < end_idx) // cautious for stash==0
				{
					for (; val_idx < existed_subkeys.size(); val_idx++) // auto &val : existed_subkeys
					{
						...
						if (val.dist < dt1 || val.max_dist == 0) //|| visited_subkeys.find(tmpsub1) != visited_subkeys.end() val.clr_idx != tmp_clrs.size() - 1 ||
							continue;
						// if (reached_subkey.find(tmpsub1) != reached_subkey.end())
						// 	continue;
	...
	// 					{
	// 						reached_subkey[tmp.sub_key] = 1;
	// 						// visited_keys.push_back({tmp.sub_key, tmp.skiplen, tmp.length}); // cautious no visit, direct gen_candidate
	// 						// gen_candidate(xxx,)
	// 						key_find kf;
	// 						gen_candidate(kf, candidate, {tmp.sub_key, tmp.skiplen, tmp.length}, visited_keys, tmp_visit, i, sub[i], dt + 1, tmp.sub_key);
	// 					}
	// 				}
	// 			}
	// 			else
	...
	// 						// ++hitliner;
	// 						// visited_keys.push_back({tmpsub1, its->skiplen, its->length}); // cautious 9-17
	// 						gen_candidate(val, candidate, {tmpsub1, its->skiplen, its->length}, visited_keys, tmp_visit, i, sub[i], dt + 1, its->sub_key);
...
void containers::test()
{
	...
	printf("zero_num=%d  combine_clr_min=%d \n", zero_num, combine_clr_min[0]);
}
...
void containers::make_clusters()
{
	...
	for (int i = 0; i < SUBINDEX_NUM; i++)
	{
		...
		printf("avg %.2f cluster 4.1 max %d min %d\n", (double)(sub_index_liner[i].size() - clr[i][clr[i].size() - 1].begin_idx) / (clr[i].size() - 1), clr_max, clr_min);
		if (clr[i].size() > 1)
			combine_clr_min[i] = ceil((double)(sub_index_liner[i].size() - clr[i][clr[i].size() - 1].begin_idx) / ((clr[i].size() - 1) * 4));
		...
		for (int c_idx = 0; c_idx < (clr[i].size()); c_idx++)
		{
			...
			bool is_comb_clr = ((end_idx - j) > combine_clr_min[i] ? true : false); //(c_idx == clr[i].size() - 1 ? true : false);//true; //
...
uint8_t *containers::lru_ids_add(uint64_t key, uint32_t sub_i, sub_info_comp comp)
{
	...
	node->key = key;
	if (comp.length > 0 && ((comp.length & MASK_LEN) * 4) > node->ids.size())
	{
