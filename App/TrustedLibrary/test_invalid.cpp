...
void get_rand_keys(vector<uint32_t> &C_0_TO_subhammdis, vector<pair<uint64_t, uint64_t>> &full_index, vector<pair<uint64_t, uint64_t>> &test_pool, int is_invalid_q)
{
    ...
    uint32_t tmp_subkey;
    full_index.clear();
    int j = 0, batch_size = 530, nums = floor(1.0 * DATA_LEN / batch_size);
    ...
    if (!is_invalid_q)
    {
        ...
        for (int t = full_index.size(); t < DATA_LEN; t++)
        {
            tmp_key[0] = 0;
            tmp_key[1] = 0;
            for (int i = 0; i < 4; i++)
    ...
    else
    {
        ...
        full_index.clear();
        while (full_index.size() < DATA_LEN)
        {
            tmp_key[0] = 0;
            tmp_key[1] = 0;
            for (int i = 0; i < 4; i++)
        ...
        printf("full_index.size(): %d test %d\n", full_index.size(), DATA_LEN);
    }

    // uint32_t sub[4];
    // for (int i = nums * 530; i < full_index.size(); i++)
    // {
    //     tmp_key[0] = full_index[i].first;
    //     tmp_key[1] = full_index[i].second;
    //     get_sub_fingerprint(sub, tmp_key);
