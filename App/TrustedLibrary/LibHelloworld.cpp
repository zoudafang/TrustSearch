...
void read_data(std::string file_name,std::vector<std::pair<uint64_t,uint64_t>> &data,std::vector<uint32_t> &data_target){
    ...
    uint64_t high, low;
    uint32_t target;
    while (data.size()<1281167&&input.read(reinterpret_cast<char*>(&high), sizeof(high)) && input.read(reinterpret_cast<char*>(&low), sizeof(low))) {
...
void send_data(std::vector<std::pair<uint64_t,uint64_t>> &data,std::vector<uint32_t> &data_target,int rand){
    ...
    while(remain_size>0){
        ...
        for(int j=0;j<send_size;j++){data_ptr[j].first+=rand;data_ptr[j].second+=rand;}
